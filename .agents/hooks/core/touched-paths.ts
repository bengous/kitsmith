...
export async function recordTouchedPaths(
  ...
  for (const filePath of touchedStatePaths(input)) {
...
export async function readTouchedPaths(input: AgentHookEvent): Promise<string[]> {
  const paths = await Promise.all(touchedStatePaths(input).map(readTouchedPathFile));
...
export async function clearTouchedPaths(input: AgentHookEvent): Promise<void> {
  await Promise.all(
    touchedStatePaths(input).map(async (filePath) => rm(filePath, { force: true })),
  );
}

export function touchedStatePath(input: AgentHookEvent): string {
  return statePath(input, stateEventKey(input));
}

export function pendingTouchedStatePath(input: AgentHookEvent): string {
  return statePath(input, "pending");
}

function touchedStatePaths(input: AgentHookEvent): string[] {
  return [...new Set([touchedStatePath(input), pendingTouchedStatePath(input)])];
}

function statePath(input: AgentHookEvent, eventKey: string): string {
  const root = repoRoot(input.cwd);
  const repoName = sanitizeStateKey(stateRootName(root));
  const identity = [input.agent, input.sessionId ?? input.transcriptPath ?? input.cwd ?? root]
    .map(stateSegment)
    .join("-");
  return path.join(
    tmpdir(),
    `${repoName}-agent-feedback`,
    `${identity}-${stateSegment(eventKey)}.json`,
  );
}
